...
class DataManageUI(QWidget):
    ...
    def initDb(self):
        ...
        try:
            ...
            self.isDbReady = False
            self.logQueue.put('未发现数据库文件，你可能未进行人脸采集')
        except Exception:
            logging.error('读取数据库异常，无法完成数据库初始化')
            self.isDbReady = False
            self.logQueue.put('读取数据库异常，初始化/刷新数据库失败')
        else:
            cursor.close()
            conn.close()
            ...
            if not self.isDbReady:
                ...
                self.logQueue.put('数据库刷新成功,检测到'+str(dbUserCount)+'人信息')
    ...
    def deleteUser(self):
        ...
        if ret == QMessageBox.Yes:
            ...
            try:
                ...
                self.logQueue.put('读写数据库异常，删除失败')
            else:
                cursor.close()
                conn.commit()
                if os.path.exists('{}/stu_{}'.format(self.datasets, stu_id)):
                    try:
                        ...
                        self.logQueue.put('删除人脸数据失败，请手动删除{}/stu_{}目录'.format(self.datasets, stu_id))
    ...
    def train(self):
        try:
            ...
            text = '将开始训练人脸数据'
            informativeText = '<b>训练过程请勿进行其它操作</b>'
...
#       抛出异常
            ...
            self.logQueue.put(repr(e))
        else:
            text = '训练完成！ 生成文件trainingData.yml'
            DataManageUI.callDialog(QMessageBox.Information, text,'', QMessageBox.Ok)
            self.logQueue.put('完成数据训练')
            self.initDb()
